...
# NUEVAS IMPORTACIONES PARA EL MODELO HÍBRIDO (SIN ALTERAR LO ANTERIOR)
...
# CONTROLES DE NAVEGACIÓN SUPERIOR (AMPLIADO A 7 DIAPOSITIVAS PARA MANTENER EL ORDEN)
...
# DIAPOSITIVA 6: MODELO HÍBRIDO (NUEVO REQUISITO INTEGRADOR PARA PÓSTER)
elif st.session_state.diapositiva == 6:
    st.markdown("""
    <div class='slide-title'>🧠 Fase Supervisada: Modelo Híbrido de Inteligencia Artificial</div>
    <div class='slide-subtitle'>Validación de la capacidad de generalización predictiva mediante Redes Neuronales (MLPClassifier)</div>
    """, unsafe_allow_html=True)
    
    if df_original is None:
        st.error("❌ No se detectó el archivo de datos necesario para procesar el modelo híbrido.")
        st.stop()
        
    # --- PIPELINE MATEMÁTICO IDÉNTICO ---
    index_cols = ['COD_MUNI', 'MUNICIPIO', 'DEPARTAMENTO']
    pivot_accion = df_original.pivot_table(index=index_cols, columns='ACCION', values='CANTIDAD', aggfunc='sum', fill_value=0)
    ...
    # K-Means (Generador automático de las etiquetas 'y')
    kmeans_hyb = KMeans(n_clusters=4, n_init=30, random_state=42)
    y_labels = kmeans_hyb.fit_predict(X_scaled_hyb)

    # Partición Científica de Filas (80% Entrenamiento y 20% Prueba) - Stratify asegura representatividad
    X_train, X_test, y_train, y_test = train_test_split(
        X_scaled_hyb, y_labels, test_size=0.2, random_state=42, stratify=y_labels
    )

    # Inicialización y ajuste del Perceptrón Multicapa (Red Neuronal)
    ...
    # --- MÉTRICAS DE CONTROL (PARA EL CUADRO DE RESULTADOS DEL PÓSTER) ---
    col_kpi1, col_kpi2, col_kpi3 = st.columns(3)
    with col_kpi1:
        st.metric(label="🎯 Precisión de Clasificación (Accuracy)", value=f"{accuracy * 100:.2f}%", help="Porcentaje de municipios que la Red Neuronal clasificó perfectamente")
    with col_kpi2:
        st.metric(label="📊 Muestra de Entrenamiento (Train)", value=f"{X_train.shape[0]} Filas", help="80% de los municipios usados para entrenar")
    with col_kpi3:
        st.metric(label="🧪 Muestra de Examen (Test)", value=f"{X_test.shape[0]} Filas", help="20% de los municipios reservados para evaluar")

    st.markdown("<br>", unsafe_allow_html=True)

    # --- SECCIÓN GRÁFICA CON EL MISMO ESTILO PREMIUM ---
    col_g1, col_g2 = st.columns(2)

    with col_g1:
        st.markdown("### A. Matriz de Confusión Estilizada")
        
        # Obtener nombres dinámicos de los clústeres presentes en test
        clústeres_reales = [f"Clúster {i}" for i in np.unique(y_test)]
        ...
        fig_cm = px.imshow(
            ...
            y=clústeres_reales,
            text_auto=True,
            title="Ajuste Operacional (Predicciones vs Clústeres Reales)",
            color_continuous_scale='Blues'
        )
        fig_cm = aplicar_estilo_premium(fig_cm)
        fig_cm.update_layout(xaxis_title="Predicción (Red Neuronal)", yaxis_title="Objetivo Real (K-Means)")
        st.plotly_chart(fig_cm, use_container_width=True)
        
        st.markdown("""
        <div class='insight-card'>
            <b>Utilidad en el Póster:</b> La concentración total de los municipios evaluados sobre la diagonal principal valida ante los jurados que la Red Neuronal asimiló con éxito el modelo matemático de distancias no supervisado.
        </div>
        """, unsafe_allow_html=True)

    with col_g2:
        st.markdown("### B. Explicabilidad del Modelo: Importancia de Variables")
        
        # Extraer coeficientes de la capa de entrada para medir la influencia relativa de las variables
        pesos_absolutos = np.sum(np.abs(mlp.coefs_[0]), axis=1)
        importancia_normalizada = (pesos_absolutos / np.max(pesos_absolutos)) * 100
        
        df_importancia = pd.DataFrame({
            'Característica': numericas_hyb,
        ...
        fig_imp = px.bar(
            ...
            y='Característica',
            orientation='h',
            title='Influencia de la Ingeniería de Características en la Red',
            color='Importancia (%)',
            color_continuous_scale='Viridis'
        )
        fig_imp = aplicar_estilo_premium(fig_imp)
        fig_imp.update_layout(xaxis_title="Peso de Impacto Relativo (%)", yaxis_title="Variables Consolidadas")
        st.plotly_chart(fig_imp, use_container_width=True)

        st.markdown("""
        <div class='insight-success'>
            <b>Sustentación Científica:</b> Este gráfico le demuestra al jurado la transparencia del modelo híbrido. Permite argumentar con precisión cuáles indicadores de la Fuerza Pública determinan el nivel de riesgo de un municipio.
        </div>
        """, unsafe_allow_html=True)

    # --- REPORTE ESTADÍSTICO ACADÉMICO ---
    st.markdown("### C. Reporte Estadístico Riguroso (Precision, Recall y F1-Score)")
    reporte_dict = classification_report(y_test, y_pred, output_dict=True)
    df_reporte = pd.DataFrame(reporte_dict).transpose().round(3)
    
    # Traducir índices numéricos a nombres formales de clúster para la tabla
    nuevos_nombres = {str(i): f"Clúster {i}" for i in range(4)}
...
# DIAPOSITIVA 7: CONCLUSIONES Y CIERRE ACADÉMICO (MOVIDO AL FINAL)
